...
namespace HermesProxy.Network.BattleNet.Session
{
    public partial class BattlenetSession
    {
        ...
        public async Task<BattlenetRpcErrorCode> HandleLogon(LogonRequest request)
        {
            ...
            if (!LocaleChecker.IsValidLocale(request.Locale.ToEnum<Locale>()))
            {
                Log.Print(LogType.Error, $"Battlenet.LogonRequest: {GetRemoteEndpoint()} attempted to log in with unsupported locale (using {request.Locale})!");
                return BattlenetRpcErrorCode.BadLocale;
            }

            _locale = request.Locale;
            _os = request.Platform;
